...
<nav aria-label="breadcrumb" class="mb-3">
    <ol class="breadcrumb mb-0">
        ...
        <li class="breadcrumb-item">
            <a href="<?= site_url('user/purchase'); ?>">My Orders</a>
        </li>
...
<div class="store-page-intro mb-4">
    ...
    <?php if ($order->status === 'pending'): ?>
        <p class="text-muted mb-0">This order is pending payment.</p>
    <?php elseif ($order->status === 'paid'): ?>
        <p class="text-muted mb-0">This order has been paid.</p>
    <?php elseif ($order->status === 'failed'): ?>
        <p class="text-muted mb-0">Payment for this order was not completed.</p>
    <?php elseif ($order->status === 'cancelled'): ?>
        <p class="text-muted mb-0">This order has been cancelled.</p>
    <?php elseif ($order->status === 'refunded'): ?>
        <p class="text-muted mb-0">This order has been refunded.</p>
    <?php else: ?>
        <p class="text-muted mb-0">View your order information below.</p>
    <?php endif; ?>
</div>
                ...
                <?php else: ?>
                    <div class="alert alert-light border small mb-4 mb-lg-0">
                        <?php if ($order->status === 'paid'): ?>
                            Payment complete for this order.
                        <?php elseif ($order->status === 'failed'): ?>
